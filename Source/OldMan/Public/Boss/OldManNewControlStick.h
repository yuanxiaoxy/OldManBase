...
#include "OldManNewControlStick.generated.h"

UCLASS()
class OLDMAN_API AOldManNewControlStick : public AOldManPullItemBase
{
    GENERATED_BODY()

public:
    AOldManNewControlStick();
    ...
    /** 碰撞组件（用于射线检测） */
    UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "ControlStick")
    ...
    bool bEnableHemisphereMode = true;

    /** 半球面高度系数（0~1），1表示完整半球 */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Hemisphere", meta = (ClampMin = "0.0", ClampMax = "1.0"))
    float HemisphereHeightFactor = 0.5f;
    ...
    /** 调试半球面颜色 */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Debug")
    FColor DebugHemisphereColor = FColor::Cyan;
    ...
    /** 处理鼠标数据（类似DraggableSplineActor的方式） */
    void HandleMouseData(const FVector& ViewDirection, float Intensity) override;
...
private:
    ...
    /** 计算半球面偏移量 */
    FVector CalculateHemisphereOffset(const FVector& PlanarOffset) const;
};