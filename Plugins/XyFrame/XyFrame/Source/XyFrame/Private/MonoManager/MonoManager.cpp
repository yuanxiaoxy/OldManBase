...
void UMonoManager::UpdateTimers()
{
    ...
    // 先收集所有需要处理的TimerId，避免在遍历过程中修改容器
    TArray<FString> AllTimerIds;
    for (const auto& TimerPair : Timers)
    {
        AllTimerIds.Add(TimerPair.Key);
    }

    TArray<FString> TimersToRemove;

    // 遍历所有TimerId，而不是直接遍历Timers容器
    for (const FString& TimerId : AllTimerIds)
    {
        // 查找Timer信息（可能在遍历过程中被删除）
...
void UMonoManager::ExecuteTimerCallback(const FString& TimerId)
{
    ...
    // 先查找所有回调，存储到局部变量中
    FTimerCallbackDelegate TimerCallback;
    if (TimerCallbacks.Contains(TimerId))
    {
        TimerCallback = TimerCallbacks[TimerId];
    }

    FTimerSimpleDelegate SimpleCallback;
    if (SimpleCallbacks.Contains(TimerId))
    {
        SimpleCallback = SimpleCallbacks[TimerId];
    }

    TFunction<void()> GenericCallback;
    FTimerInfo* TimerInfo = Timers.Find(TimerId);
    if (TimerInfo && TimerInfo->GenericCallback)
    {
        GenericCallback = TimerInfo->GenericCallback;
    }
